...
# Load packages
pacman::p_load(purrr,tidyverse,lme4,lmerTest)
...
read_pitch <- function(filename) {
    ...
    file$f0 <- as.numeric(file$f0)
    file$time <- as.numeric(file$time)
    
    # parse filename - extract study, diagnosis, subject and trial
    list1 <- str_match(filename, "Study([\\d]+)D([\\d]+)S([\\d]+)T([\\d]+)")
    
    # extract pitch descriptors (mean, sd, iqr, etc)
    list2 <- file %>% summarize(
      sd = sd(f0),
      mean = mean(f0),
      IQR = IQR(f0),
      list2 = min(f0),
    ...
    # column binder info from summary statistics and filename into df
    data <- cbind(list1,list2)
    
    # rename column names
    x <- c("file", "study", "diagnosis", 
           "subject", "trial", "sd_pitch", 
           "mean_pitch", "iqr_pitch", 
           "min_pitch", "max_pitch")
    colnames(data) <- x
...
pitch_data = list.files(path = "C:/Users/askes/Dropbox/University/Cognitive Science/3. semester/Experimental Methods 3/Portfolio-3/Pitch",pattern = ".txt", full.names = T) %>% 
  
  # Takes every file in the list, applier the read_pitch function and rbinder the data it returns.
    purrr::map_df(read_pitch)


# Write the collected data to csv.
write.csv(pitch_data,"C:/Users/askes/Dropbox/University/Cognitive Science/3. semester/Experimental Methods 3/Portfolio-3/pitch_data.csv", row.names = FALSE)
...
#Add numbers to the different studies, because participants have the same numbers.
#demo$Participant <- ifelse(demo$Study == 5, demo$Participant + 1000, demo$Participant)
#demo$Participant <- ifelse(demo$Study == 6, demo$Participant + 2000, demo$Participant)
#demo$Participant <- ifelse(demo$Study == 7, demo$Participant + 3000, demo$Participant)

#change column names
colnames(demo)[1] <- "study"
colnames(demo)[3] <- "diagnosis"

# Change diagnosis to dummy variable (0 = control, 1 = SZ)
...
# Diagnosis as numeric so its the same in demo and pitch_data
pitch_data$diagnosis <- as.numeric(pitch_data$diagnosis)

#Add numbers to the different studies, because participants have the same numbers.
#pitch_data$subject <- ifelse(pitch_data$study == "5", pitch_data$subject + 1000, pitch_data$subject)
#pitch_data$subject <- ifelse(pitch_data$study == "6", pitch_data$subject + 2000, pitch_data$subject)
#pitch_data$subject <- ifelse(pitch_data$study == "7", pitch_data$subject + 3000, pitch_data$subject)

#Change name so they are called soundname and participant in both art and pitch_data
#change column names
colnames(pitch_data)[1] <- "soundname"
colnames(pitch_data)[4] <- "Participant"


#Merge demo and pitch_data
data <- merge(demo, pitch_data, key = c("Participant","diagnosis","study"))
# Merge data and articulation
data <- merge(data, art, by = "soundname")


# Add to columns with unique pair ID and unique ID.
...
#Summary of data grouped by diagnosis
data %>% group_by(diagnosis) %>% 
  summarize(
      ...
      mean_SAPS = mean(SAPS),
      sd_SANS = sd(SAPS),
      mean_V_IQ = mean(VerbalIQ),
      mean_NV_IQ = mean(NonVerbalIQ),
      mean_T_IQ = mean(TotalIQ)
)

#Summary of data grouped by study
data %>% group_by(study) %>% 
  summarize(
      n_F = sum(Gender == "F"),
      n_M = sum(Gender == "M"),
...
```{r}
# Add pause duration column
data$PauseDuration <- ifelse(data$npause == 0, 0,
                             (data$dur..s. - data$phonationtime..s.)/(data$npause))


# Making a list to scale
colnames(data)
list_for_scaling <- (c("sd_pitch", "mean_pitch", "iqr_pitch", "min_pitch", "max_pitch", "Education", "SANS", "SAPS", "VerbalIQ", "NonVerbalIQ", "TotalIQ", "nsyll", "npause", "dur..s.", "phonationtime..s.", "speechrate..nsyll.dur.","articulation.rate..nsyll...phonationtime.","ASD..speakingtime.nsyll.", "PauseDuration"))

# Group by study
data <- group_by(data, study)

# Scaling function
scale2 <- function(x, na.rm = FALSE) (x - mean(x, na.rm = na.rm)) / sd(x, na.rm)

# Manually making ASD speaking time as numeric (due to debogging)
data$ASD..speakingtime.nsyll. <- as.numeric(data$ASD..speakingtime.nsyll.)

# Running the scaling on the data including the new column pauseDuration and grouped by Language.
grouped_scaled_data <- data %>% group_by(Language) %>% mutate_at(list_for_scaling, scale2)
...
```{r}
# Write the collected data to csv.
write.csv(grouped_scaled_data,"C:/Users/askes/Dropbox/University/Cognitive Science/3. semester/Experimental Methods 3/Portfolio-3/grouped_scaled_data.csv", row.names = FALSE)
```

# Create models
```{r}
# modelling with random slope and intercept

# model 1: spoken properties
spoken_property <- lmer(phonationtime..s. ~ diagnosis + (1+diagnosis|uPairID), grouped_scaled_data, REML = F)
summary(spoken_property)

# model 2: pitch
pitch <- lmer(iqr_pitch ~ diagnosis + (1+diagnosis|uPairID), grouped_scaled_data, REML = F)
summary(pitch)

# model 3: speech rate
speech_rate <- lmer(speechrate..nsyll.dur. ~ diagnosis + (1+diagnosis|uPairID), grouped_scaled_data, REML = F)
summary(speech_rate)

# model 4: pause duration
pause_dur <- lmer(PauseDuration ~ diagnosis + (1+diagnosis|uPairID), grouped_scaled_data, REML = F, control = lmerControl(optimizer = "nloptwrap", calc.derivs = F))
summary(pause_dur)
```
